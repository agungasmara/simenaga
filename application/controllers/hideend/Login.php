...
class Login extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->user->loggedin) {
			redirect(base_url()."/hideend/admin");
		}
		$this->template->loadContent("/hidepage/login/index.php", array());
	}
    	...
    	if($this->settings->info->activate_account)
    	{
    		if(!$r->active) {
    			$this->template->error(lang("error_72") . "<a href='".
    				site_url("hideend/register/send_activation_code/" . $r->ID . "/" .
    				 urlencode($r->email)).
	...
	public function google_login()
	{
		...
		$client->setRedirectUri(site_url("hideend/login/google_login"));
		...
		if (isset($_GET['code'])) {
			...
			$redirect = 'https://'.$_SERVER['HTTP_HOST'].'/hideend/login/google_login';
		...
		if ($client->getAccessToken()) {
			...
			//echo '<pre>'; print_r( $peop['modelData']['emails'][0]['value'] );die; //test
			$username = $peop['modelData']['name']["givenName"].$peop['modelData']['name']["familyName"];
			$username = str_replace(" ", "_", $username);
			$email = $peop['modelData']['emails'][0]['value'];
			$image = $peop['modelData']['image']['url'];
			$oauth_id = $peop['id'];
			...
			redirect(site_url("/hideend/admin"));
		} else {
		    $authUrl = $client->createAuthUrl();
		   //echo '<pre>'; print_r( $authUrl);die; //test
		    redirect($authUrl);
		}
	}
	...
	public function facebook_login()
	{
		...
		$loginUrl = $helper->getLoginUrl(site_url("hideend/login/facebook_login_pro"));
	...
	public function facebook_login_pro()
	{
		...
		if(isset($accessToken)) {
			...
			//echo '<pre>'; print_r($userNode);die; //test	
  			$provider = "facebook";
	...
	public function twitter_login()
	{
		...
		$request_token = $twitteroauth->oauth("oauth/request_token",
			array("oauth_callback" => site_url("hideend/login/twitter_login_pro")));
	...
	public function twitter_login_pro()
	{
		 ...
		 && !empty($_SESSION['oauth_token_secret']))
		{
		    ...
		    redirect(site_url("hideend/login/twitter_login"));
		}
	}
	...
	public function logout($hash)
	{
		...
		redirect(base_url()."hideend/login");
	}
	...
	public function forgotpw_pro()
	{
		...
		$email_template->message = $this->common->replace_keywords(array(
			...
			"[SITE_URL]" => site_url(),
			"[EMAIL_LINK]" =>
				site_url("hideend/login/resetpw/" . $token . "/" . $user->ID),
		...
		redirect(site_url("hideend/login/forgotpw"));
	}
